...
protected void onCreate(Bundle savedInstanceState) {
        ...
        urlFinal = ("https://yma-server.herokuapp.com/search/"+searchQuery);
        }
...
public void onClick(DialogInterface dialogInterface, int i) {
        if(i==0){
        AlertDialog.Builder createBuilder = new AlertDialog.Builder(SearchResultsActivity.this);
        LayoutInflater inflater = SearchResultsActivity.this.getLayoutInflater();
...
public void onClick(DialogInterface dialogInterface, int i) {
        ...
        Toast.makeText(SearchResultsActivity.this, "Added to playlist", Toast.LENGTH_SHORT).show();
        }
        }
...
class fetchData extends  Thread {
    ...
    public void run() {
        ...
        try {
            ...
            if (!data.isEmpty()) {
                ...
                String topTitle = top.getString("title");
                JSONArray topThumb = top.getJSONArray("thumbnails");
                JSONObject topThumbnail = topThumb.getJSONObject(0);
                String topThumbnailUrl = topThumbnail.getString("url");
                String topThumbnailFinal = UrlClean.url(topThumbnailUrl);
                JSONArray data = jsonObject.getJSONArray("songs");
                JSONArray albumData = jsonObject.getJSONArray("albums");
                ...
                for (int i =0; i< data.length(); i++){
                    ...
                    String trackName = dataFinal.getString("title");
                    String id = dataFinal.getString("id");
                    String  author = dataFinal.getString("author");
                    JSONArray songThumb = dataFinal.getJSONArray("thumbnails");
                    ...
                    for (int l = 0; l < 1; l++) {
                        Song song = new Song(trackName, author, id, "Greece", songThumbnailFinal);
                        Song album = new Song(albumName, albumAuthor, id, "Greece",albumThumbnailFinal);
                ...
                uiHandler.post(new Runnable() {
                    @Override
                    public void run() {
                            imageView= findViewById(R.id.TopResultImage);
                        Picasso.get().load(topThumbnailFinal).placeholder(R.drawable.missingbackground).error(R.drawable.missingbackground).fit().centerCrop().into(imageView);

                        TextView artist = findViewById(R.id.TopResultTitle);
                        artist.setText(topTitle);
                    }
                });
            }
    ...
    private void setupBottomNavigation(){
        ...
        BottomNavigationHelper.enableNavigation(SearchResultsActivity.this,bottomNavigationViewEx);
